...
每个脚本有**三个命名槽**，外加**一个自动槽**。它们都存在 `.checkpoints/`，该目录**已被 git 忽略**——永远不会与你的仓库历史冲突。

| 槽位 | 谁写入 | 用途 |
|---|---|---|
| 1 / 2 / 3 | 你 | 你主动保留的命名存档点 |
| **auto** | 插件 | **每次不可逆操作之前**自动写入的保险 |

**自动槽不是第四个命名槽。** 它不能被手动选择、命名或占用——由插件自己在不可逆操作前写入，存在的意义就是让你**不必记得**去存档：

- **从代码导入**——积木会按文件重建，手工搭的积木被替换。导入前的模型先入自动槽。
- **优化积木**——优化器会改你的代码。快照在**运行之前**就写好，不管这次运行能否通过它自己的检查——出问题时你需要的正是这个返回点。
- **释放 / 取消管理**——积木文件会被删除。 模型在删除**之前**就从文件里读出并存入自动槽，所以释放同样可恢复。

**什么时候该自己存命名槽**

- 大型*积木 → 代码*写入之前，如果你有一阵子没提交了。
- 跨多个方法的结构性重构之前——自动槽只保留**最后一次**动作，一连串编辑会把它反复覆盖。
- 把文件交给别人之前，或让 agent 会话动它之前。

**自动槽不是什么。** 它只留一份快照，不是历史：下一次不可逆操作就会替换它。想要一个下周还能回来的点，就存命名槽——它们就是干这个的。
...
装了助手之后，解释积木不再停在"里面套着 3 个积木"：她会走一遍子积木，逐个念出来——分支体、`else` 分支、以及插在值槽里的积木。

**这些上限是有意的：**

| 上限 | 值 | 原因 |
|---|---|---|
| 每次回答的积木数 | 8 | 大方法否则就是一堵文字墙，而且每次点击都要付代价 |
| 单个节点的子积木 | 6 | 一个二十输入的节点会吃掉全部预算 |
| 深度 | 3 | "这块积木是什么"问的不是更深的结构 |

**提前停下时会明说。** 剩余数量会被统计并报出来，她会让你选中那些积木单独再看——不会悄悄丢掉。

**翻译是数据。** 树句子放在独立的 `tree.*` 槽组里，所以没翻译该组的语言仍然能得到积木主句；补一份树翻译就是改 `neko.json`。英语、简体中文、繁体中文已翻译。

**解释还会读逻辑。** 念完树之后，她会说这块积木对数据做了什么：

> *"也就是在 `health > 0` 成立的时候，它会把 `health`、`timer` 改变喵。"*

| 部分 | 来源 |
|---|---|
| 条件 | 名为 `cond` 的插槽，填入输入后渲染出来。`foreach` 没有条件，改用 `source` |
| 目标 | 积木自己的 `target` / `name` 插槽，加上分支体或循环体里每个语句的同类插槽——嵌套分支也算 |
| 动作数 | 方法体里跑了多少条语句 |

**八种语言全都能用，不需要任何按语言写的代码。** 解析器完全不看语法，只读两样各语言积木本来就填好的东西：`node` 字段（`while` / `for` / `foreach` / `if` / `assign` / `localDecl` / `call` / `return` / `expr`）和插槽名（`cond`、`target`、`name`、`source`）。新语言只要积木沿用同样的命名，就自动获得逻辑解读。

如果一块积木什么都不改，她会直说而不是沉默；句子按种类分键（`logic.loop.*`、`logic.branch.*`、`logic.action.*`），各语言可以分别翻译。她**不会**尝试解释"为什么需要这块积木"——那需要语言模型；她只报告图里看得见的东西。

### 6.7 下一个积木的建议怎么排序（1.0.3）

栈尾的建议条由三个信号排序：

1. **类别先验**——声明与控制流优先于惯用块，自动生成的 API 最后；
2. **本方法已用过**——文件里在用的，就是还会再用的；
3. **邻接转移打分**——从图里读"积木 A 后面跟积木 B"（继续、分支体、`else`），在本文件里已经跟在当前积木后面的候选得分更高。

第三个信号来自文件自身的数据，不是内置词典：它会随脚本自适应，也不需要训练。
...
| `Optimise Blocks` | 对选中脚本运行优化器（与健康度窗口同一次运行，同一套门禁与回滚） |
...
健康度窗口底部会列出已注册的 pass；只要存在优化器，就会出现 **`Optimise blocks`** 按钮。

**跑的是什么**——两个 pass，按注册顺序：

1. **`optimizer.constantcondition`**——把 `if (true) { … }` 折叠成 `{ … }`，把
   `if (false) { … }` 折叠成空。条件必须是 `true`/`false` 字面量（直接写或经字面量块）：
   字面量没有副作用，所以取用分支不会改变行为。`if` 之后的代码会接到所取分支的尾部，
   被孤立的字面量留给第 2 个 pass。
2. **`optimizer.deadcode`**——删除同一列表中 `return` / `break` / `continue`
   之后的语句，以及已经没有任何引用的节点（编辑残留）。

**为什么安全。** 可达性从方法入口开始遍历，遇到终止语句即停止：未被标记的节点在任何输入下都执行不到，删掉不会改变行为。

**门禁。** 优化会改你的代码，所以绝不单独相信它：

1. 运行前先把模型深拷贝一份；
2. 运行后所有引用必须可解析，图必须无环；
3. 模型必须能打印，且**错误数不比运行前多**（本来就不完美的文件不会因为"仍然不完美"被拒）；
4. 打印出来的文本必须能重新解析且无错误。

任一条不通过就整体回滚，原因显示在按钮下方。通过时按钮会报告删了多少积木、涉及多少方法。

**只有通过才写盘。** 没通过门禁、或什么都没改，都不会写文件。结束后积木编辑器会自动刷新。
