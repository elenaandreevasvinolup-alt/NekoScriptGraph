...
namespace NekoScriptGraph
{
    ...
    public static class Nsg_L10n
    {
        ...
        static readonly Dictionary<string, string> English = new Dictionary<string, string>
        {
            ...
            { "health.optimizerReady"     , "Auto-optimisation is ready: {0} optimizer pass(es) registered." },
            { "health.optimizerNone"      , "No optimizer passes are registered. The hook is ready: register an INsg_Pass(Optimizer)." },
            { "health.passes"             , "registered passes: {0}" },
            { "health.optimizerDeadCode"  , "Remove unreachable statements" },
            { "health.optimizerConstCond" , "Fold constant conditions" },
            { "health.optimizeRun"        , "Optimise blocks" },
            { "health.optimizeNothing"    , "Nothing to optimise: no unreachable statements found." },
            { "health.optimizeDone"       , "Removed {0} unreachable block(s) across {1} method(s)." },
            { "health.optimizeRolledBack" , "Rolled back: {0}" },
            { "optimise.selectScript"     , "Select a script first. Optimisation runs on one file at a time; it rewrites that file's blocks." },
            { "optimise.noModel"          , "This script has no block model yet. Take it under management first, then optimise." },
            ...
            { "msg.modelLoadFailed"       , "the block file {0} could not be read ({1}); it was treated as empty. Fix or delete the .nsg.json beside the script, then re-import." },
            ...
            { "palette.advanced"          , "Show advanced blocks" },
            { "manage.noLanguages"        , "No language is loaded, so there is no source file to look for. Check that the LanguageSupport folder is intact, then use Blocks ▸ Languages: Show Loaded." },
            { "manage.noSourcesProject"   , "Nothing to take under management: the project has no source files with these extensions — {0}." },
            { "manage.noSourcesFolder"    , "Nothing to take under management in {0}: no source files with these extensions — {1}." },
            { "manage.allManaged"         , "All {0} source file(s) here are already under management. Use Release to undo, or edit them in the block editor." },
            { "palette.other"             , "Other" },
